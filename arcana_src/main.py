...
def arcana():
    ...
    def importMsgs(status, prompt: str):
        ...
        if "error" in status:
            print(
                "    " +
                prompt +
                " Module ┠╴\x1b[31mERROR\x1b[39;49m\n──────────────────────┸────────────────────────────────────────────\n\x1b[31m" +
                str(e) +
                "\x1b[0m")
            if "." in status:
                print(
            ...
            if "." in status:
                print("    " + prompt + " Module ┖╴\x1b[32mSUCCESS\x1b[39;49m")
            else:
                print("    " + prompt + " Module ┠╴\x1b[32mSUCCESS\x1b[39;49m")
    try:
        import os
    ...
    try:
        ...
        importMsgs("error", "Random")
    importMsgs("success", "Random")
    try:
        import getpass
    except Exception as e:
        importMsgs("error", "Getpass")
    importMsgs("success", "Getpass")
    try:
        import time
    except Exception as e:
        importMsgs("error", "Time")
    importMsgs("success", "Time")
    try:
        import subprocess
    except Exception as e:
        importMsgs("error", "Subprocess")
    importMsgs("success", "Subprocess")
    try:
        from modules.b2eC import b2e, arcanaid
    except Exception as e:
        importMsgs("error", "B2E-C")
                ...
                if b2e.decode(str(encode), msgs["channel"]) == "VALID":
                    try:
                        if b2e.decode(
                                ...
                                datetime.now().strftime("%X")):
                            dateP = "Today - " + \
                                b2e.decode(str(encode), msgs["deltatime"])
                        else:
                            dateP = b2e.decode(str(encode) + b2e.decode(str(encode),
                                                                        msgs["deltatime"]),
                                               msgs["date"]) + " - " + b2e.decode(str(encode),
                                                                                  msgs["deltatime"])
                        print(
                            dateP +
                            "\n" +
